      ...
      <div className=" max-w-4xl mx-auto sm:my-32 my-12 p-8 md:p-0">
        <div className="sm:mb-32 mb-16 ">
          ...
          <p className="dark:text-zinc-400 text-zinc-600 sm:text-lg  mb-8">
            ...
            helping me become an expert in my primary focus and a well-rounded developer
            overall! 🌍🔝
          </p>
        </div>
        ...
        <div>
            ...
            {Object.entries(FourTierMasteryData).map(([year, tiers], index) => (
                <div key={index} className="border-2 rounded-md overflow-hidden  py-2">
                    <h2 className="text-orange-600 font-semibold sm:text-2xl border-b-2 pb-2 text-center"> {year} </h2>
                    <div className="p-2 divide-y-2 ">
                        {tiers.map((tier, index) => (
                            <div key={index} className="py-4 group dark:hover:bg-orange-900/20 cursor-pointer transition-all duration-700 rounded-md p-4">
                                <h3 className=" text-lg font-roboto-slab">{tier['name']} : </h3>
                                <div className="pl-4">
                                    <h4 className="text-lg font-jersey dark:text-zinc-100 group-hover:text-orange-600 " >{tier['which']}</h4>
